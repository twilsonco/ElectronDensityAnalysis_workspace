...
"""
    split_path_at_point(path, point)

Finds the closest node in `path` to `point`, and splits the array into two halves.
Injects `point` exactly at the boundary so both halves terminate perfectly at the intersection.
"""
function split_path_at_point(path::Vector{SVector{3,Float64}}, point::SVector{3,Float64})
    # Find the discrete node closest to the true mathematical intersection
    closest_idx = argmin([sum(abs2, p - point) for p in path])

    # Half 1: Start -> Intersection
    part1 = path[1:max(1, closest_idx - 1)]
    push!(part1, point)

    # Half 2: Intersection -> End
    part2 = [point]
    append!(part2, path[min(length(path), closest_idx + 1):end])
...
"""
    predict_topology_voronoi_quads(maxima, origin, radius)

Computes the Spherical Delaunay triangulation of the Maxima.
Returns: 
1. `pred_minima`: The coordinates of the predicted minima (Voronoi vertices).
2. `quads`: A list of TopologicalQuads, representing the intersecting Max-Max and Min-Min pairs.
"""
function predict_topology_voronoi_quads(
    maxima::Vector{SVector{3,Float64}}, origin::SVector{3,Float64}, radius::Float64
)
    n = length(maxima)
    if n < 3
        return SVector{3,Float64}[], TopologicalQuad[]
    end

    # Store valid Delaunay triangles
    ...
    for i in 1:n, j in (i + 1):n, k in (j + 1):n
        ...
        # Normal to the plane defined by the 3 points
        ...
        # Orient normal OUTWARD relative to the sphere origin
        if dot(dir, p1 - origin) < 0
            dir = -dir
        end

        # Empty Circumcircle Test: Check if all other points are "below" this plane
        is_delaunay = true
        for m in 1:n
            if m == i || m == j || m == k
                ;
            ...
            # If dist > epsilon, the point is inside the circumcircle
            if dist > 1e-7
                is_delaunay = false
                break
            end
        ...
        if is_delaunay
            ...
            # The predicted minimum is the circumcenter projected to the sphere surface
            push!(pred_minima, origin + dir * radius)
        end
    end

    # 2. Extract Topological Quads (Adjacency Map)
    # Map each edge to the indices of the triangles (minima) that share it
    edge_to_triangles = Dict{SVector{2,Int},Vector{Int}}()

    for (tri_idx, tri) in enumerate(delaunay_triplets)
        # The 3 edges of this triangle
        edges = [
        ...
        for e in edges
            if !haskey(edge_to_triangles, e)
                edge_to_triangles[e] = Int[]
            end
            push!(edge_to_triangles[e], tri_idx)
        end
    end

    # 3. Build Quads
    quads = TopologicalQuad[]
    for (edge, tri_indices) in edge_to_triangles
        # A valid interior edge on a closed sphere must be shared by exactly 2 triangles
        if length(tri_indices) == 2
            push!(quads, TopologicalQuad(
                edge[1],         # Max 1
                edge[2],         # Max 2
                tri_indices[1],  # Min 1 (Index into pred_minima)
                tri_indices[2],   # Min 2 (Index into pred_minima)
            ))
        end
    end

    println(
        "Voronoi Dual Extraction: Found $(length(pred_minima)) minima and $(length(quads)) intersecting quads.",
    )

    return pred_minima, quads
end

"""
    resolve_saddles_and_skeleton(maxima, minima, quads, mesh, sampler)

Generates the full Morse-Smale 1-skeleton. 
Saddles are defined strictly as the intersection of optimized Ridge and Valley splines.
"""
function resolve_saddles_and_skeleton(
    maxima::Vector{SVector{3,Float64}},
    minima::Vector{SVector{3,Float64}},
    quads::Vector{TopologicalQuad},
    mesh::SphericalScalarMesh,
    sampler::FieldSampler,
)
    ...
    println("Resolving $(length(quads)) Saddles via Spline Intersection...")

    for (i, q) in enumerate(quads)
        # 1. Fetch Anchors
        p_max1 = maxima[q.max1_idx]
        p_max2 = maxima[q.max2_idx]
        p_min1 = minima[q.min1_idx]
        p_min2 = minima[q.min2_idx]

        # 2. Optimize Global Splines
        ridge_path = optimize_spline_connector(p_max1, p_max2, mesh, sampler; type=:ridge)
        valley_path = optimize_spline_connector(p_min1, p_min2, mesh, sampler; type=:valley)

        # 3. Find Intersection (The True Saddle)
        saddle_pos = find_spline_intersection(
            ridge_path, valley_path, mesh.origin, mesh.radius
        )
        push!(saddles, saddle_pos)
        s_idx = length(saddles)

        # 4. Split Paths at the Saddle
        r1, r2 = split_path_at_point(ridge_path, saddle_pos)
        v1, v2 = split_path_at_point(valley_path, saddle_pos)

        # 5. Store Connectors (Enforcing consistent orientation: Extremum -> Saddle)
        # r1 goes from max1 to saddle
        # r2 goes from saddle to max2 (so we reverse it to go max2 -> saddle)
        connectors[(:max_sad, q.max1_idx, s_idx)] = r1
        connectors[(:max_sad, q.max2_idx, s_idx)] = reverse(r2)

        # Same for valleys
        connectors[(:min_sad, q.min1_idx, s_idx)] = v1
        connectors[(:min_sad, q.min2_idx, s_idx)] = reverse(v2)
    end

    return saddles, connectors
end
...
function analyze_condensed_density(
    ...
    if n_anchors >= 3
        println("Regime: Molecular")
        # Extract Minima and the Quad relationships
        pred_min, quads = predict_topology_voronoi_quads(
            anchors, smooth_mesh.origin, smooth_mesh.radius
        )

        println("--- 3. Relaxing Minima ---")
        # We still relax the minima first so the Valley splines have accurate endpoints
        refined_min = relax_minima_constrained(
            pred_min, sampler, smooth_mesh.origin, smooth_mesh.radius; stiffness=1.0
        )

        # Optional: clean_degenerate_topology would go here if needed, 
        # but it requires re-mapping quad indices. For now, try running without it 
        # as the splines might naturally resolve tight minima clusters.

        println("--- 4. Resolving Saddles and Skeleton ---")
        saddles, connectors = resolve_saddles_and_skeleton(
            anchors, refined_min, quads, smooth_mesh, sampler
        )

    elseif n_anchors == 1
        println("Regime: Terminal")
        v_max = anchors[1] - smooth_mesh.origin
        push!(refined_min, smooth_mesh.origin - v_max)
    end

    clean_min, clean_sad = clean_degenerate_topology(
        refined_min, saddles, smooth_mesh.origin, smooth_mesh.radius; tol_factor=0.15
    )

    # Construct Final Topology Object
    topo = ChemicalTopology(anchors, clean_min, clean_sad, connectors)
