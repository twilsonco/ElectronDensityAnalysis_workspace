...
function test_comprehensive_cu_system()
    ...
    try
        sys = load_data_scm_band_t41(
            "/Users/haiiro/NoSync/scm_kf_reader/data/Cu.t41"; load_var="rho"
        )[1]
    ...
    comprehensive_checkpoint = "gbd_checkpoints/cu/comprehensive_cu_1000dgb.jls"
    ...
    if system_gbd_data === nothing
        ...
        try
            system_gbd_data = gradient_bundle_decomposition(
                sys;
                cps=cps,
                num_gbs=20000,                    # 1000 DGBs per atom for comprehensive analysis
                interp_type=Linear,
                ...
                checkpoint_dir="gbd_checkpoints/cu",
    ...
    try
        open_plt_file(
            "comprehensive_cu_system_complete_band.plt",
        ...
        file_size = filesize("comprehensive_cu_system_complete_band.plt")
...
if test_passed
    ...
    println("\nGenerated files:")
    println(
        "  📁 comprehensive_cu_system_complete_band.plt - Complete system with node-centered spheres",
    )
    println(
