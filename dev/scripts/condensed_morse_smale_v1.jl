...
function process_sphere(input_file_path::String)
    ...
    println("$input_file_basename Computed Discrete Gradient:")
    println("  Critical Vertices (Minima): $(length(discrete_grad.critical_vertices))")
    println("  Critical Edges (Saddles): $(length(discrete_grad.critical_edges))")
    println("  Critical Triangles (Maxima): $(length(discrete_grad.critical_triangles))")
    println("  Total Vertices: $(length(spherical_mesh.positions))")
    println("  Total Edges: $(length(spherical_mesh.edges))")
    println("  Total Triangles: $(length(spherical_mesh.triangles))")
    check_poincare_hopf(discrete_grad)
    # println("Plotting critical points...")
    ...
    display(
        plot_results(
            ...
            discrete_grad;
            skeleton=skeleton,
            plot_title=" $input_file_basename (Original)",
        ),
    )

    range = maximum(spherical_mesh.values) - minimum(spherical_mesh.values)
    # println("Scalar Field Range: $range")
    # println("Applying Anisotropic Smoothing...")
    ...
    smoothed_skeleton = extract_skeleton(smoothed_discrete_grad, smoothed_mesh)
    # println("Plotting smoothed critical points...")

    display(
        plot_results(
            smoothed_mesh,
            smoothed_discrete_grad;
            skeleton=smoothed_skeleton,
            plot_title=" $input_file_basename (Smoothed)",
        ),
    )
    ...
    best_skeleton = extract_skeleton(best_grad, smoothed_mesh)
    display(
        plot_results(
            smoothed_mesh,
            best_grad;
            skeleton=best_skeleton,
            plot_title=" $input_file_basename (Binary Search Threshold=$(round(best_threshold, digits=8)))",
        ),
    )

    println("\n" * "="^120)
    println("$input_file_basename SIMPLIFICATION RESULT (Binary Search)")
    println("="^120)
    println("Selected threshold: $(round(best_threshold, digits=12))")
    println("Target counts     : minima=$target_min saddles=$target_sad maxima=$target_max")
    println("Observed counts   : minima=$best_min saddles=$best_sad maxima=$best_max")
    println("="^120)

    # Print mesh statistics
    println("\n" * "="^70)
    println("$input_file_basename MESH FUNCTION VALUE STATISTICS")
    println("="^70)
    println("Minimum: $min_val")
    println("Maximum: $max_val")
    println("Mean:    $mean_val")
    println("Median:  $median_val")
    println("Std Dev: $std_val")
    println("="^70)
end

function main(
    input_dir::String="/Users/haiiro/NoSync/ElectronDensityAnalysis.jl/gbd_checkpoints/ethane_20000",
)
    if !isdir(input_dir)
        error("Directory does not exist: $input_dir")
    end

    spherical_mesh_files = sort(
        filter(name -> endswith(name, "_spherical_mesh.jls"), readdir(input_dir))
    )

    if isempty(spherical_mesh_files)
        println("No *_spherical_mesh.jls files found in: $input_dir")
        return nothing
    end

    for filename in spherical_mesh_files
        process_sphere(joinpath(input_dir, filename))
    end
end
