...
#
# This variant includes INTERIOR points only (excludes mirror-plane boundary).
# For all points (interior + boundary), change the loop bounds to i=0:q, j=0:q-i.
...
println("\nWedge points (q=$q, interior only): $(length(cart_pts))")
...
function plot_oh_wedge_on_sphere(q::Int)
    ...
    ax  = Axis3(fig[1, 1];
        ...
        title  = "Oh irreducible wedge  (q = $q, interior only)",
    )
